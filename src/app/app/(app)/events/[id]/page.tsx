...
export default function EventDetail() {
  ...
  async function rsvpShift(shiftId: string) {
    ...
    await supabase
      .from("shift_signups")
      .upsert({ shift_id: shiftId, status: "going" });
    await loadAll();
    setSaving(false);
  }
  ...
  async function cancelShift(shiftId: string) {
    ...
    await supabase
      .from("shift_signups")
      .upsert({ shift_id: shiftId, status: "cancelled" });
    await loadAll();
    setSaving(false);
  }
  ...
  async function openRoster(shiftId: string) {
    ...
    setQrUrl("");
    // roster (with skills aggregated in API)
  ...
  async function showQr(shiftId: string) {
    const r = await fetch(`/api/shifts/${shiftId}/issue-checkin-token`, {
      method: "POST",
    });
  ...
  return (
    <div style={{ padding: 16, display: "grid", gap: 14, maxWidth: 900 }}>
      ...
      <div>
        ...
        {shifts.length === 0 && (
          <div style={{ color: "#666" }}>No shifts yet.</div>
        )}
            ...
            return (
              ...
              >
                <div>
                  ...
                  <div
                    style={{ fontSize: 12, color: full ? "#b91c1c" : "#666" }}
                  >
      ...
      {rosterShiftId && (
        ...
        >
          ...
          <div style={{ marginTop: 12, display: "grid", gap: 8 }}>
            {roster.length === 0 && (
              <div style={{ color: "#666" }}>No signups yet.</div>
            )}
            {roster
              .filter((r) => {
                if (!filterSkillId) return true;
                return r.skill_badges?.includes(
                  allSkills.find((s) => s.id === filterSkillId)?.name || "_none_"
                );
              })
              .map((r) => (
                <div
                ...
                >
                  <div>
                    ...
                    <div style={{ fontSize: 12, marginTop: 6 }}>
                      ...
                      {r.checked_in_at && (
                        <> • Checked in: {new Date(r.checked_in_at).toLocaleString()}</>
                      )}
                    </div>
                  </div>
                  ...
                  {(role === "admin" || role === "moderator") && (
                    <div style={{ display: "grid", gap: 6 }}>
                      <button
                        onClick={() =>
                          markAttendance(rosterShiftId!, r.user_id, true)
                        }
                        style={{
                          border: "1px solid #ddd",
                          borderRadius: 8,
                      ...
                      <button
                        onClick={() =>
                          markAttendance(rosterShiftId!, r.user_id, false)
                        }
                        style={{
                          border: "1px solid #ddd",
                          borderRadius: 8,
